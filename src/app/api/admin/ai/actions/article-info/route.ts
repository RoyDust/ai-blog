...
function normalizeArticleInfo(outputs: Partial<Record<PostAiAction, unknown>>) {
  ...
  const slug = generatePostSlug(readOptionalString(slugOutput.slug));
  const excerpt = readOptionalString(summaryOutput.summary);
  const seoDescription = readOptionalString(seoOutput.seoDescription);

  if (!slug || !excerpt || !seoDescription) {
    throw new ValidationError("AI 返回的信息不完整");
  }
...
async function POSTHandler(request: Request) {
  try {
    ...
    const results: Array<{ itemId: string; action: PostAiAction; modelId: string | null; output: unknown }> = [];
    const failures: Array<{ action: PostAiAction; message: string }> = [];
    ...
    for (const action of ARTICLE_INFO_ACTIONS) {
      ...
      try {
        ...
        results.push({ itemId: item.id, action, modelId: result.modelId ?? null, output: result.output });
    ...
    if (failures.length > 0) {
      return NextResponse.json(
        {
          success: false,
          error: failures[0]?.message ?? "一键 AI 生成失败",
          data: { taskId: task.id, failures, items: results },
        },
        { status: 502 },
      );
    }

    let articleInfo: ReturnType<typeof normalizeArticleInfo>;

    try {
      articleInfo = normalizeArticleInfo(outputs);
    } catch (error) {
      return NextResponse.json(
        {
          success: false,
          error: error instanceof Error ? error.message : "AI 返回的信息不完整",
          data: { taskId: task.id, items: results },
        },
        { status: 502 },
      );
    }
